...
class User(db.Model):
    ...
    role = db.Column(
        db.Enum("client", "coach", "both", "admin", name="user_roles"), nullable=True
    )
    status = db.Column(
        db.Enum("active", "disabled", name="user_status"), default="active"
    )
    ...
    fitness_survey = db.relationship(
        "FitnessSurvey", backref="user", uselist=False, cascade="all, delete-orphan"
    )
...
class FitnessSurvey(db.Model):
    ...
    fitness_level = db.Column(
        db.Enum("beginner", "intermediate", "advanced", name="fitness_levels")
    )
...
class RoleChangeRequest(db.Model):
    ...
    requested_role = db.Column(
        db.Enum("coach", "both", name="requested_role_enum"), nullable=False
    )
...
class ClientRequest(db.Model):
    ...
    status = db.Column(
        db.Enum("pending", "accepted", "denied", name="request_status"),
        default="pending",
    )
    ...
    coach = db.relationship(
        "User", foreign_keys=[coach_id], backref="received_requests"
    )
...
class CoachApplication(db.Model):
    ...
    status = db.Column(
        db.Enum("pending", "approved", "denied", name="coach_application_status"),
        default="pending",
    )
...
class ModerationReport(db.Model):
    ...
    report_type = db.Column(
        db.Enum("coach", "chat", name="moderation_report_type"), nullable=False
    )
    ...
    relationship_id = db.Column(
        db.Integer, db.ForeignKey("coach_relationships.id", ondelete="CASCADE")
    )
    reason = db.Column(db.String(255), nullable=False)
    details = db.Column(db.Text)
    status = db.Column(
        db.Enum(
            "open", "reviewed", "resolved", "dismissed", name="moderation_report_status"
        ),
        default="open",
    )
    reviewed_by = db.Column(db.Integer, db.ForeignKey("users.id"))
    ...
    reporter = db.relationship(
        "User", foreign_keys=[reporter_id], backref="reports_created"
    )
    ...
    def to_dict(self):
        return {
            ...
            "created_at": self.created_at.isoformat() if self.created_at else None,
            "reviewed_at": self.reviewed_at.isoformat() if self.reviewed_at else None,
            "reporter": self.reporter.to_dict(include_profile=True)
            if self.reporter
            else None,
            "reported_user": self.reported_user.to_dict(include_profile=True)
...
class CoachRelationship(db.Model):
    ...
    status = db.Column(
        db.Enum("active", "ended", name="relationship_status"), default="active"
    )
    ...
    client = db.relationship(
        "User", foreign_keys=[client_id], backref="client_relationships"
    )
    coach = db.relationship(
        "User", foreign_keys=[coach_id], backref="coach_relationships"
    )
...
class Review(db.Model):
    ...
    # Relationships
    client = db.relationship(
        "User", foreign_keys=[client_id], backref="reviews_written"
    )
    ...
    def to_dict(self, include_client=False):
        ...
        if include_client and self.client:
            data["client"] = {
                ...
                "profile": self.client.profile.to_dict()
...
class ChatMessage(db.Model):
    ...
    relationship_id = db.Column(
        db.Integer,
        db.ForeignKey("coach_relationships.id", ondelete="CASCADE"),
        nullable=False,
    )
    ...
    def to_dict(self, include_sender=False):
        ...
        if include_sender and self.sender:
            data["sender"] = {
                ...
                "profile": self.sender.profile.to_dict()
    ...
    def __repr__(self):
        return (
            f"<ChatMessage relationship={self.relationship_id} sender={self.sender_id}>"
        )
...
class Exercise(db.Model):
    ...
    difficulty = db.Column(
        db.Enum("beginner", "intermediate", "advanced", name="exercise_difficulty")
    )
    video_url = db.Column(db.String(255))
    instructions = db.Column(db.Text)
    created_by = db.Column(
        db.Integer, db.ForeignKey("users.id")
    )  # NULL for default exercises, user_id for custom
    is_public = db.Column(
        db.Boolean, default=True
    )  # Public exercises vs. coach-specific
    ...
    is_library_workout = db.Column(
        db.Boolean, default=False
    )  # True = complete workout in the library, False = movement/component
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
...
class WorkoutPlan(db.Model):
    ...
    status = db.Column(
        db.Enum("active", "completed", "archived", name="plan_status"), default="active"
    )
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    updated_at = db.Column(
        db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow
...
class WorkoutPlanAssignment(db.Model):
    ...
    workout_day_id = db.Column(
        db.Integer, db.ForeignKey("workout_days.id", ondelete="SET NULL")
    )
...
class CalendarNote(db.Model):
    """Personal notes attached to a calendar date"""
    ...
    user = db.relationship("User", backref="calendar_notes")
    ...
    def __repr__(self):
        return f"<CalendarNote user={self.user_id} date={self.date}>"
...
class PlanExercise(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    workout_day_id = db.Column(
        db.Integer, db.ForeignKey("workout_days.id", ondelete="CASCADE"), nullable=False
    )
...
class WorkoutLog(db.Model):
    ...
    plan_id = db.Column(
        db.Integer, db.ForeignKey("workout_plans.id", ondelete="SET NULL")
    )
    workout_day_id = db.Column(
        db.Integer, db.ForeignKey("workout_days.id", ondelete="SET NULL")
    )
    # Library-backed or custom workout fields
    library_exercise_id = db.Column(
        db.Integer, db.ForeignKey("exercises.id", ondelete="SET NULL")
    )
    workout_name = db.Column(db.String(200))
    calories_burned = db.Column(db.Integer)
    exercise_type = db.Column(
        db.String(50)
    )  # Mirrors Exercise.category for ad-hoc logs
...
class ExerciseLog(db.Model):
    ...
    workout_log_id = db.Column(
        db.Integer, db.ForeignKey("workout_logs.id", ondelete="CASCADE"), nullable=False
    )
    exercise_id = db.Column(db.Integer, db.ForeignKey("exercises.id"), nullable=False)
...
class MealLog(db.Model):
    ...
    date = db.Column(db.Date, nullable=False)
    meal_type = db.Column(
        db.Enum("breakfast", "lunch", "dinner", "snack", name="meal_types")
    )
...
class BodyMetric(db.Model):
    ...
    def to_dict(self):
        return {
            ...
            "body_fat_percentage": float(self.body_fat_percentage)
            if self.body_fat_percentage
            else None,
            "muscle_mass_kg": float(self.muscle_mass_kg)
            if self.muscle_mass_kg
            else None,
...
class WellnessLog(db.Model):
    ...
    date = db.Column(db.Date, nullable=False)
    mood = db.Column(
        db.Enum("excellent", "good", "okay", "poor", "terrible", name="mood_levels")
    )
    ...
    sleep_quality = db.Column(
        db.Enum("excellent", "good", "fair", "poor", name="sleep_quality")
    )
...
class PaymentRecord(db.Model):
    ...
    coach = db.relationship(
        "User", foreign_keys=[coach_id], backref="payments_received"
    )
