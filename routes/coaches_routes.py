...
def _normalize_pricing_items(items):
    ...
    for i, raw in enumerate(items):
        ...
        if st is None or (isinstance(st, str) and not st.strip()):
            return (
                None,
                f"session_type is required for row {i} (e.g. Hourly, Weekly package)",
            )
...
def get_public_top_coaches():
    ...
    try:
        ...
        return success_response(
            {"coaches": coaches_out}, "Top coaches retrieved successfully", 200
        )
    ...
    try:
        ...
        for user in paginated.items:
            try:
                ...
                # Add specializations
                specializations = CoachSpecialization.query.filter_by(
                    coach_id=user.id
                ).all()
...
def get_coach_reviews(coach_id):
    ...
    try:
        ...
        paginated = (
            Review.query.filter_by(coach_id=coach_id)
            .order_by(Review.created_at.desc())
...
def send_hire_request(coach_id):
    ...
    try:
        ...
        existing_request = ClientRequest.query.filter_by(
            client_id=client_id, coach_id=coach_id, status="pending"
        ).first()
...
def get_my_requests():
    ...
    try:
        ...
        if request_type == "received":
            ...
            requests = (
                ClientRequest.query.filter_by(coach_id=user_id)
                .order_by(ClientRequest.requested_at.desc())
                .all()
            )
        else:
            # Get requests sent as a client
            requests = (
                ClientRequest.query.filter_by(client_id=user_id)
                .order_by(ClientRequest.requested_at.desc())
                .all()
            )
        ...
        return success_response(
            {"requests": requests_data}, "Requests retrieved successfully", 200
        )
...
def respond_to_request(request_id):
    ...
    try:
        ...
        if status == "accepted":
            relationship = CoachRelationship(
                client_id=hire_request.client_id, coach_id=coach_id, status="active"
            )
            db.session.add(relationship)
        ...
        return success_response(
            hire_request.to_dict(), f"Request {status} successfully", 200
        )
...
def submit_review(coach_id):
    ...
    try:
        ...
        relationship = CoachRelationship.query.filter_by(
            client_id=client_id, coach_id=coach_id
        ).first()

        if not relationship:
            return error_response(
                "You must have a relationship with this coach to leave a review", 403
            )
...
def get_my_clients():
    ...
    try:
        ...
        return success_response(
            {"clients": clients}, "Clients retrieved successfully", 200
        )
...
def get_my_coach():
    ...
    try:
        ...
        coach_data["start_date"] = (
            relationship.start_date.isoformat() if relationship.start_date else None
        )

        # Add coach survey info
        coach_survey = CoachSurvey.query.filter_by(
            user_id=relationship.coach.id
        ).first()
        if coach_survey:
            coach_data["coach_info"] = coach_survey.to_dict()
        else:
    ...
    try:
        ...
        if application:
            ...
            application = CoachApplication(
                user_id=user_id, notes=data.get("notes"), status="pending"
            )
            db.session.add(application)

        db.session.commit()
        return success_response(
            application.to_dict(), "Coach application submitted successfully", 201
        )
    except Exception as e:
        db.session.rollback()
    ...
    try:
        ...
        if request.method == "GET":
            ...
            return success_response(
                ...
                "Coach settings retrieved successfully",
                200,
            )
        ...
        if "specialization_ids" in data and isinstance(
            ...
            for specialization_id in data["specialization_ids"]:
                db.session.add(
                    CoachSpecialization(
                        coach_id=coach_id, specialization_id=specialization_id
                    )
                )
...
def get_client_progress(client_id):
    ...
    try:
        ...
        body_metrics = (
            BodyMetric.query.filter_by(user_id=client_id)
            .order_by(BodyMetric.date.desc())
            .limit(12)
            .all()
        )
        wellness_logs = (
            WellnessLog.query.filter_by(user_id=client_id)
            .order_by(WellnessLog.date.desc())
            .limit(14)
            .all()
        )
        workout_logs = (
            WorkoutLog.query.filter_by(client_id=client_id)
        ...
        return success_response(
            ...
            "Client progress retrieved successfully",
            200,
        )
...
def get_my_pricing():
    ...
    try:
        ...
        rows = (
            CoachPricing.query.filter_by(coach_id=coach_id)
            .order_by(CoachPricing.id)
            .all()
        )
        return success_response(
...
def get_my_coach_profile():
    ...
    try:
        ...
        if not user or user.role not in ("coach", "both"):
            return error_response(
                "Coach profile is only available for coach accounts", 403
            )
...
def update_my_coach_profile():
    ...
    try:
        ...
        if not user or user.role not in ("coach", "both"):
            return error_response(
                "Only coaches can update coach profile information", 403
            )
        ...
        if "experience_years" in data:
            ...
            if ey is None or ey == "":
                ...
                try:
                    ...
                    return error_response(
                        "experience_years must be a whole number", 400
                    )
                if ey_int < 0 or ey_int > 80:
                    return error_response(
                        "experience_years must be between 0 and 80", 400
                    )
